    ...
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      {statsBuckets.length === 0 ? (
        <div className="text-center py-24 bg-white rounded-2xl border border-dashed border-slate-200">
          ...
          <p className="text-slate-500 font-medium">暂无将截止的任务</p>
        </div>
      ) : (
          ...
          {statsBuckets.map(bucket => {
            ...
            return (
              <div key={bucket.key} className="relative mb-5">
                {showDayOverload && (
                  <div className="mb-2 px-3 py-2 rounded-lg bg-yellow-100 text-yellow-800 text-sm font-semibold">
                    ⚠️ 当日有 {bucket.tasks.length} 个任务将截止，建议优先处理！
                  </div>
                )}
